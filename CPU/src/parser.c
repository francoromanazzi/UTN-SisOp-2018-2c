...
t_operacion* parse(char* linea){
	...
	if(!strcmp(argv[0], "abrir")){
		...
		dictionary_put(ret->operandos, "path", (void*) argv[1]);
	}
	...
	/* ASIGNAR path linea datos */
		...
		dictionary_put(ret->operandos, "path", (void*) argv[1]);
		dictionary_put(ret->operandos, "linea", (void*) argv[2]);
		dictionary_put(ret->operandos, "datos", (void*) argv[3]);
	}

	/* WAIT recurso */
	else if(!strcmp(argv[0], "wait")){
		ret->tipo_operacion = OP_WAIT;
		dictionary_put(ret->operandos, "recurso", (void*) argv[1]);
	}
	...
	/* CREAR path cant_lineas */
		...
		dictionary_put(ret->operandos, "path", (void*) argv[1]);
		dictionary_put(ret->operandos, "cant_lineas", (void*) argv[2]);
	}
